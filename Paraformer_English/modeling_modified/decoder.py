...
class DecoderLayerSANM(torch.nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None, cache=None):
        x = self.feed_forward(self.norm1(tgt))
        if self.self_attn:
            x = tgt + self.self_attn(self.norm2(x))
        if self.src_attn is not None:
            x = x + self.src_attn(self.norm3(x), memory, ret_attn=False)
        return x, memory
...
class ParaformerSANMDecoder(BaseTransformerDecoder):
    ...
    def __init__(
        ...
        if self.normalize_before:
            ...
            self.after_norm.requires_grad_(False)
        if use_output_layer:
    ...
    def forward(
            ...
            hlens: torch.Tensor = None,
            ys_in_lens: torch.Tensor = None,
            chunk_mask: torch.Tensor = None,
            return_hidden: bool = False,
            return_both: bool = False,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        x, memory = self.decoders(ys_in_pad, hs_pad)
        x, _ = self.decoders3(x, memory)
        return self.output_layer(self.after_norm(x))
