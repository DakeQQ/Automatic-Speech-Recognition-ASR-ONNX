...
def encode(
        positions: torch.Tensor = None, depth: int = None, dtype: torch.dtype = torch.float32
):
...
class SANMEncoder(nn.Module):
    ...
    def __init__(
        ...
        positions = torch.arange(1, 502, dtype=torch.int32).unsqueeze(0)   # 502 Means max audio input = 30 seconds
