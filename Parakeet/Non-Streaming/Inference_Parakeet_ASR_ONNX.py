...
Auto-detects the graph set in the target folder (targeted-fused, raw, or separately optimized) and runs a
single full-sequence encoder pass followed by the Token-and-Duration Transducer (TDT) greedy decode. The
decoder/joint carries its LSTM state across steps through persistent ORT buffers and
...
# Edit the constants below for demo, audio, and ONNX Runtime behavior.
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tokenizer-path", type=Path, default=None,
                        help="Optional tokenizer.json path; defaults to tokenizer.json in the model folder.")
    return parser.parse_args()


_ARGS = _parse_args()

_GRAPH_NAMES = ("ASR_Metadata.onnx", "Parakeet_ASR_Encoder.onnx", "Parakeet_ASR_Decoder.onnx")
_DEFAULT_CANDIDATES = (
    _SCRIPT_DIR / "Parakeet_ASR_ONNX_Fused",
...
def _resolve_graphs():
    ...
    for folder in candidates:
        if all((folder / name).is_file() for name in _GRAPH_NAMES):
            return folder, _GRAPH_NAMES
    return candidates[0], _GRAPH_NAMES
...
_DECODER_DISABLED_OPTIMIZERS = ["FuseFp16InitializerToFp32NodeTransformer"]


def _make_session(path) -> onnxruntime.InferenceSession:
    disabled_optimizers = (
        _DECODER_DISABLED_OPTIMIZERS
        if Path(path).name == DECODER_NAME
        else None
    )
...
def main() -> None:
    ...
    max_symbols_per_step = int(meta["max_symbols_per_step"])
    tokenizer_name = "tokenizer.json"

    tokenizer_path = (
        _ARGS.tokenizer_path.expanduser().resolve()
        if _ARGS.tokenizer_path is not None
        else ONNX_FOLDER / tokenizer_name
    )
    tok = Tokenizer.from_file(str(tokenizer_path))

    # Decoder/joint I/O and persistent state buffers.
    ...
    frame_idx_np = filled_for(dj_in_specs[dj_frame_in], axes={0: 1})
    frame_idx_ort = _ort_from_numpy(frame_idx_np)
    token_buf = _ort_from_numpy(
        filled_for(dj_in_specs[dj_token_in], blank_id, axes={0: 1, 1: 1})
    )
    state_h_buf = _ort_from_numpy(
        filled_for(dj_in_specs[dj_state_h_in], axes={1: 1})
    )
    state_c_buf = _ort_from_numpy(
        filled_for(dj_in_specs[dj_state_c_in], axes={1: 1})
    )
    ...
    duration_buf = _ort_from_numpy(
        filled_for(dj_out_specs[dj_duration_out], axes={0: 1, 1: 1})
    )
    ...
    enc_in_specs = metadata_by_name(sess_enc.get_inputs())
    enc_audio_meta = enc_in_specs[_in_names(sess_enc)[0]]
    ...
    audio = array_for(enc_audio_meta, audio, axes={0: 1, 1: 1, 2: audio.shape[2]})
    ...
    frame_idx = 0
    steps = 0
    max_steps = max_symbols_per_step * n_frames
    while frame_idx < n_frames and steps < max_steps:
        ...
        if int(is_blank_buf.numpy().flat[0]) == 0:
            token_id = int(next_token_buf.numpy().flat[0])
            if token_id == eos_id:
                break
