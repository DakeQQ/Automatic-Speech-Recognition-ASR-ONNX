...
def _parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Run Qwen ASR ONNX inference.")
	parser.add_argument("--onnx-folder", "--model-folder", dest="onnx_folder", type=Path, default=_SCRIPT_DIR / "Qwen_ASR_Optimized", help="Folder containing ONNX graphs, for example Qwen_ASR_Optimized or Qwen_ASR_ONNX.")
...
# ============================================================================
USE_NORMALISE_AUDIO    = False              # Apply RMS loudness normalisation before feeding the model.
...
def main() -> None:
	...
	if _model_meta.get("qwen_asr_metadata_version") != "1":
		raise ValueError(
			f"Required Qwen_ASR metadata version 1 is missing from {onnx_model_Metadata}. "
			"Re-export with Export_Qwen_ASR.py to stamp the model metadata."
		)
	...
	def _meta_int_list(key: str) -> List[int]:
		value = _model_meta.get(key)
		if value is None:
			raise KeyError(
				f"Required metadata key '{key}' is missing from {onnx_model_Metadata}. "
	...
	EXPECTED_NUM_LAYERS = _meta_int("num_layers")
	EXPORTED_MAX_BEAM_SIZE = _meta_int("max_beam_size")
	print(
		f"  Model metadata: {len(_model_meta)} keys "
		f"(sample_rate={SAMPLE_RATE}, input_audio_length={MAX_INPUT_AUDIO_LENGTH}, "
		f"max_seq_len={MAX_SEQ_LEN}, stop_token_ids={STOP_TOKEN})."
	)
	...
	mask_dtype_np = np.float16 if "float16" in main_inputs_meta[main_attention_mask_idx].type else np.float32
	...
	# The audio input dtype is taken straight from the encoder's audio input tensor in the ONNX
	# model, so it always matches how the model was exported: "int16" -> raw PCM (the graph divides
	# by 32768), "float16"/"float" -> audio pre-normalised to [-1, 1] here.
	audio_input_type = ort_session_Encoder._inputs_meta[0].type
	if "int16" in audio_input_type:
		input_audio_dtype = "INT16"
	elif "float16" in audio_input_type:
		input_audio_dtype = "F16"
	else:
	...
	if num_layers_rt != EXPECTED_NUM_LAYERS:
		raise ValueError(
			f"Decoder Main graph exposes {num_layers_rt} KV layers but metadata num_layers="
			f"{EXPECTED_NUM_LAYERS}. Re-export all Qwen_ASR graphs together."
		)
	...
	top_k = TOP_K
	beam_size = BEAM_SIZE
	if beam_size > EXPORTED_MAX_BEAM_SIZE:
		raise ValueError(
			f"BEAM_SIZE={beam_size} exceeds exported max_beam_size={EXPORTED_MAX_BEAM_SIZE}. "
			"Re-export with a larger MAX_BEAM_SIZE or lower BEAM_SIZE."
		)
	...
	attention_mask_buf = _ort_zeros((1, 1, 1, 1, 1), mask_dtype_np)
