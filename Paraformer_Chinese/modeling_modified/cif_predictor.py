...
class CifPredictorV2(torch.nn.Module):
    def __init__(
        ...
        self.cif_conv1d.requires_grad_(False)
        self.cif_output = torch.nn.Linear(idim, 1)
        self.cif_output.requires_grad_(False)
        self.dropout = torch.nn.Dropout(p=dropout)
        self.threshold = threshold
        self.smooth_factor = smooth_factor
        self.noise_threshold = noise_threshold
    ...
    def tail_process_fn(self, hidden, alphas, token_num=None, mask=None):
        alphas = torch.cat([alphas, self.tail_threshold_expand], dim=-1)
        hidden = torch.cat([hidden, self.zeros], dim=1)
