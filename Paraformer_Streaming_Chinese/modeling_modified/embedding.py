...
class StreamSinusoidalPositionEncoder(torch.nn.Module):
    ...
    def __int__(self, d_model=80, dropout_rate=0.1):
        positions = torch.arange(1, 502, dtype=torch.int32).unsqueeze(0)  # 502 = Max 30 seconds audio
        self.position_encoding = self.encode(positions, 560).half()
    ...
    def forward(self, x, start_idx, end_idx):
        return x + self.position_encoding[:, start_idx:end_idx]
