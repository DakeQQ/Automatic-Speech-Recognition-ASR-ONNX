...
class WhisperFlashAttention2(WhisperAttention):
    """
    Whisper_V2 flash attention module. This module inherits from `WhisperAttention` as the weights of the module stays
...
# Copied from transformers.models.mbart.modeling_mbart.MBartEncoderLayer with MBart->Whisper_V2, MBART->WHISPER
...
WHISPER_INPUTS_DOCSTRING = r"""
    Args:
        ...
        decoder_input_ids (`torch.LongTensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            ...
            Whisper_V2 uses the `decoder_start_token_id` as the starting token for `decoder_input_ids` generation. If
...
class WhisperDecoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperConfig):
        ...
        self.save_key_de = [None] * len(self.layers)
        self.save_value_de = [None] * len(self.layers)
    ...
    def forward(
        ...
        past_key_de = past_key_de.float()
        past_value_de = past_value_de.float()
        past_key_en = past_key_en.float()
        past_value_en = past_value_en.float()
        for idx, decoder_layer in enumerate(self.layers):
            hidden_states, self.save_key_de[idx], self.save_value_de[idx] = decoder_layer(
                hidden_states,
                attention_mask=attention_mask,
                past_key_de=past_key_de[idx],
                past_value_de=past_value_de[idx],
                past_key_en=past_key_en[idx],
                past_value_en=past_value_en[idx],
                encoder_hidden_states=None,
                layer_head_mask=None,
        ...
        return hidden_states, torch.stack(self.save_key_de, dim=0), torch.stack(self.save_value_de, dim=0)
...
@add_start_docstrings(
    "The bare Whisper_V2 Model outputting raw hidden-states without any specific head on top.",
...
class WhisperModel(WhisperPreTrainedModel):
    ...
    def freeze_encoder(self):
        """
        Calling this function will disable the gradient computation for the Whisper_V2 encoder so that its parameters will
...
@add_start_docstrings(
    "The Whisper_V2 Model with a language modeling head. Can be used for automatic speech recognition.",
    WHISPER_START_DOCSTRING,
)
...
@add_start_docstrings(
    """
    Whisper_V2 decoder with a language modeling head on top (linear layer with weights tied to the input embeddings).
    """,
    WHISPER_START_DOCSTRING,
)
...
@add_start_docstrings(
    """
    Whisper_V2 Encoder Model with a sequence classification head on top (a linear layer over the pooled output) for tasks
