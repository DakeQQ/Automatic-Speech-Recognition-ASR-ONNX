...
ORT_Accelerate_Providers = []                               # If you have accelerate devices for : ['CUDAExecutionProvider', 'TensorrtExecutionProvider', 'CoreMLExecutionProvider', 'DmlExecutionProvider', 'OpenVINOExecutionProvider', 'ROCMExecutionProvider', 'MIGraphXExecutionProvider', 'AzureExecutionProvider']
                                                            # else keep empty.
provider_options = []
DYNAMIC_AXES = True                                         # The default dynamic_axes is the input audio length. Whisper_V2 series models only support dynamic_axes due to their transformer structure.
...
STOP_TOKEN = 50257                                          # 50257 is the end token for common Whisper_V2 series model.
...
class WHISPER_ENCODER(torch.nn.Module):
    ...
    def forward(self, audio):
        ...
        mel_features = torch.matmul(self.fbank, torch.sqrt(real_part * real_part + imag_part * imag_part)).clamp(min=1e-10).log10()
...
class WHISPER_DECODER(torch.nn.Module):
    def __init__(self, whisper, max_seq_len):
...
with torch.inference_mode():
    ...
    whisper_decoder = WHISPER_DECODER(model, MAX_SEQ_LEN)
    input_ids = torch.tensor([50258, get_language_id(TARGET_LANGUAGE), get_task_id(TASK, False)[0]], dtype=torch.int32)
    ...
    del history_len
    del attention_mask
print('\nExport done!\n\nStart to run Whisper_V2 by ONNX Runtime.\n\nNow, loading the model...')
...
for language_idx, test in enumerate(test_audio):
    ...
    # Start to run Whisper_V2
    slice_start = 0
    slice_end = INPUT_AUDIO_LENGTH
    input_ids = np.array([50258, get_language_id(language), get_task_id(TASK, False)[0]], dtype=np.int32)
