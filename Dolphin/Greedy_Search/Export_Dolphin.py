...
class DOLPHIN_DECODER(torch.nn.Module):
    ...
    def forward(self, *all_inputs):
        ...
        max_logit_ids = torch.argmax(logits[:, language_start: language_end], dim=-1, keepdim=True).int()
...
ids_40002 = onnxruntime.OrtValue.ortvalue_from_numpy(np.array([40002], dtype=np.int64), 'cpu', 0)

init_past_keys_B = onnxruntime.OrtValue.ortvalue_from_numpy(np.zeros((ort_session_B._outputs_meta[0].shape[0], ort_session_B._outputs_meta[0].shape[1], 0), dtype=np.float32), 'cpu', 0)
init_past_values_B = onnxruntime.OrtValue.ortvalue_from_numpy(np.zeros((ort_session_B._outputs_meta[num_layers].shape[0], 0, ort_session_B._outputs_meta[num_layers].shape[2]), dtype=np.float32), 'cpu', 0)
