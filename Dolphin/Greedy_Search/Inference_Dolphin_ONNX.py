...
ids_40002 = onnxruntime.OrtValue.ortvalue_from_numpy(np.array([40002], dtype=np.int64), device_type, DEVICE_ID)

if device_type != 'dml':
    init_past_keys_B = onnxruntime.OrtValue.ortvalue_from_numpy(np.zeros((ort_session_B._outputs_meta[0].shape[0], ort_session_B._outputs_meta[0].shape[1], 0), dtype=model_dtype), device_type, DEVICE_ID)
    init_past_values_B = onnxruntime.OrtValue.ortvalue_from_numpy(np.zeros((ort_session_B._outputs_meta[num_layers].shape[0], 0, ort_session_B._outputs_meta[num_layers].shape[2]), dtype=model_dtype), device_type, DEVICE_ID)
else:
    init_past_keys_B = onnxruntime.OrtValue.ortvalue_from_numpy(np.zeros((ort_session_B._outputs_meta[0].shape[0], ort_session_B._outputs_meta[0].shape[1], 0), dtype=model_dtype), 'cpu', 0)
    init_past_values_B = onnxruntime.OrtValue.ortvalue_from_numpy(np.zeros((ort_session_B._outputs_meta[num_layers].shape[0], 0, ort_session_B._outputs_meta[num_layers].shape[2]), dtype=model_dtype), 'cpu', 0)
...
for test in test_audio:
    ...
    while slice_end <= aligned_len:
        ...
        if detect_language:
            ...
            lang_id = onnxruntime.OrtValue.numpy(all_outputs_B[-2])[0, 0] + 7
            for i in range(num_layers):
                input_feed_B[in_name_B[i]] = init_past_keys_B
            for i in range(num_layers, num_keys_values):
                input_feed_B[in_name_B[i]] = init_past_values_B
        ...
        if detect_region:
            ...
            all_outputs_B = ort_session_B.run_with_ort_values(out_name_B, input_feed_B)
            region_id = onnxruntime.OrtValue.numpy(all_outputs_B[-2])[0, 0] + 145
            for i in range(num_layers):
                input_feed_B[in_name_B[i]] = init_past_keys_B
            for i in range(num_layers, num_keys_values):
                input_feed_B[in_name_B[i]] = init_past_values_B
        ...
        input_feed_B[in_name_B[-3]] = ids_40002
