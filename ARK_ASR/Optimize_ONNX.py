"""Optimize ARK-ASR donors and rebuild its merged Encoder+Prefill bundle.
...
ORIGINAL_FOLDER_PATH  = str(_SCRIPT_DIR / "ARK_ASR_ONNX")
OPTIMIZED_FOLDER_PATH = str(_SCRIPT_DIR / "ARK_ASR_Optimized")
...
def _main_nodes_to_exclude(model_path: str) -> list[str] | None:
    ...
    if len(matches) != 1 or not matches[0]:
        raise RuntimeError(
            f"Expected one named ARK-ASR lm-head projection; found {matches}."
        )
    return matches
...
MODEL_PLANS: dict[str, Plan] = {
    ...
    "ARK_ASR_Prefill_Greedy":         Plan(method="Q8", process=False, optimize=True, transformer=True),
    "ARK_ASR_Prefill_Penalty_Greedy": Plan(method="Q8", process=False, optimize=True, transformer=True),
    "ARK_ASR_PrefillSampling":        Plan(method="Q8", process=False, optimize=True, transformer=True),
    "ARK_ASR_Decode_Greedy":          Plan(method="Q8", process=False, optimize=True, transformer=True),
    "ARK_ASR_Decode_Penalty_Greedy":  Plan(method="Q8", process=False, optimize=True, transformer=True),
    "ARK_ASR_DecodeSampling":         Plan(method="Q8", process=False, optimize=True, transformer=True),
    "ARK_ASR_SharedInitializers":     Plan(method="Q8", process=False, optimize=True, transformer=True),
...
def _inspect_embed_layout(
    ...
    if len(gather_indices) != 1:
        raise RuntimeError(
            "ARK-ASR Embed must contain exactly one standard Gather; "
    ...
    if len(gather.input) < 2 or len(gather.output) != 1:
        raise RuntimeError("ARK-ASR Embed Gather has an unexpected input/output contract.")
    ...
    if table_index is None:
        raise RuntimeError("ARK-ASR Embed Gather does not read a constant table.")
    table = model.graph.initializer[table_index]
    if len(table.dims) != 2:
        raise RuntimeError(
            f"ARK-ASR Embed table must be rank 2, got {list(table.dims)}."
        )
    ...
    if gather_axis != 1:
        raise RuntimeError(
            f"ARK-ASR Embed Gather axis must be 0 or 1, got {gather_axis}."
        )
...
def _share_block_quantized_embed_with_main(
    ...
    if (
        ...
        raise RuntimeError("Raw ARK-ASR Embed and lm-head tables are not exactly tied.")
    ...
    if raw_embed_external or raw_main_external:
        if raw_embed_external != raw_main_external:
            raise RuntimeError(
                "Raw ARK-ASR Embed and lm-head tables reference different bytes."
            )
    elif raw_table.raw_data != raw_lm_head.raw_data:
        raise RuntimeError("Raw ARK-ASR Embed and lm-head tables are not byte-identical.")
...
def _share_float_embed_with_main(
    ...
    if (
        ...
        raise RuntimeError("Raw ARK-ASR Embed and lm-head tables are not tied.")
    raw_embed_external = _external_data_map(raw_table)
    raw_main_external = _external_data_map(raw_lm_head)
    if raw_embed_external or raw_main_external:
        if raw_embed_external != raw_main_external:
            raise RuntimeError("Raw ARK-ASR Embed and lm-head reference different bytes.")
    elif raw_table.raw_data != raw_lm_head.raw_data:
        raise RuntimeError("Raw ARK-ASR Embed and lm-head are not byte-identical.")
...
def _share_tied_embed_with_main(
    ...
    raise RuntimeError(
        f"Tied ARK-ASR Embed does not support Main method {main_plan.method!r}."
    )
