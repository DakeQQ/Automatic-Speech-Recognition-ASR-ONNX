"""Build Audio8-ASR merged ONNX graphs backed by one shared initializer bundle.

Each prefill graph composes the audio encoder, Audio8 placeholder injection,
...
PREFILL_GREEDY_MODEL_NAME = "Audio8_ASR_Prefill_Greedy.onnx"
PREFILL_PENALTY_GREEDY_MODEL_NAME = "Audio8_ASR_Prefill_Penalty_Greedy.onnx"
PREFILL_SAMPLING_MODEL_NAME = "Audio8_ASR_PrefillSampling.onnx"
DECODE_GREEDY_MODEL_NAME = "Audio8_ASR_Decode_Greedy.onnx"
DECODE_PENALTY_GREEDY_MODEL_NAME = "Audio8_ASR_Decode_Penalty_Greedy.onnx"
DECODE_SAMPLING_MODEL_NAME = "Audio8_ASR_DecodeSampling.onnx"
SHARED_MODEL_NAME = "Audio8_ASR_SharedInitializers.onnx"
...
def save_shared_initializers_from_tensors(shared: dict[str, TensorProto], path: Path) -> None:
    ...
    graph = onnx.helper.make_graph([], "audio8_asr_shared_initializers", [], [], initializer=references)
    model = onnx.helper.make_model(
        graph,
        producer_name="Audio8_ASR/Shared_Merged.py",
...
def extend_prefill_rotary_initializers(
    shared: dict[str, TensorProto],
    source_folder: Path,
    min_elements: int = MIN_SHARED_INITIALIZER_ELEMENTS,
    model_file_names: dict[str, str] | None = None,
) -> int:
    """Add immutable prefill-rotary state under its merged-graph namespace."""
    rotary = prefixed(
        load_model(
            source_folder / _model_file_name(model_file_names, "rotary_prefill")
        ),
        "prefill_",
    )
    return extend_shared_initializers(
        shared,
        rotary,
        min_elements,
        component_label=_model_file_name(model_file_names, "rotary_prefill"),
    )


def extend_decode_rotary_initializers(
    shared: dict[str, TensorProto],
    source_folder: Path,
    min_elements: int = MIN_SHARED_INITIALIZER_ELEMENTS,
    model_file_names: dict[str, str] | None = None,
) -> int:
    """Add immutable decode-rotary state under its merged-graph namespace."""
    rotary = prefixed(
        load_model(
            source_folder / _model_file_name(model_file_names, "rotary_decode")
        ),
        "decode_",
    )
    return extend_shared_initializers(
        ...
        rotary,
        min_elements,
        component_label=_model_file_name(model_file_names, "rotary_decode"),
    )
...
def build_prefill_frontend(
    ...
    """Compose Audio8 Encoder plus placeholder-aware embedding injection."""
    if encoder is None:
        encoder = load_model(
    ...
    if len(encoder.graph.input) != 2 or len(encoder.graph.output) != 1:
        raise RuntimeError(
            "Audio8 Encoder must expose audio and input_ids inputs plus one bridge output."
        )
    if len(concat.graph.input) != 3 or len(concat.graph.output) != 2:
        raise RuntimeError(
            "Audio8 Concat_Embed must expose base_embed, audio_hidden, input_ids, "
            "then concat_embed and concat_len."
        )
    encoder_component = prefixed_preserving_initializers(encoder, "encoder_")
    concat_component = prefixed(concat, "concat_")
    encoder_audio = f"encoder_{encoder.graph.input[0].name}"
    encoder_ids = f"encoder_{encoder.graph.input[1].name}"
    encoder_hidden = f"encoder_{encoder.graph.output[0].name}"
    concat_audio = f"concat_{concat.graph.input[1].name}"
    concat_base = f"concat_{concat.graph.input[0].name}"
    concat_ids = f"concat_{concat.graph.input[2].name}"
    ...
    frontend = merge_models_no_check(
        encoder_component,
        concat_component,
        [(encoder_hidden, concat_audio), (encoder_ids, concat_ids)],
    )
    rename_tensor(frontend, encoder_audio, "audio")
    rename_tensor(frontend, encoder_ids, "input_ids")
    rename_tensor(frontend, concat_base, "base_embed")
...
def merge_models_no_check(
    ...
    merged.producer_name = "Audio8_ASR/Shared_Merged.py"
...
def delete_obsolete_strategy_artifacts(
    ...
    old_family = "Be" + "am"
    legacy_names = (
        f"Audio8_ASR_First_{old_family}_Search.onnx",
        f"Audio8_ASR_Second_{old_family}_Search.onnx",
        f"Audio8_ASR_Prefill_{old_family}.onnx",
        f"Audio8_ASR_Decode_{old_family}.onnx",
        f"Audio8_ASR_Decode_Penalty_{old_family}.onnx",
    )
    protected = {
...
def dedup_embed_into_shared_lm_head(
    ...
    (``[hidden, vocab]`` vs ``[vocab, hidden]``) and, because Audio8-ASR ties
...
def prepare_embed_table_for_sharing(
    ...
    tied to Main's ``lm_head``.  When a checkpoint leaves them untied, register the heavy
    ``[vocab, hidden]`` table in ``shared`` (mutated in place) so it is streamed once into
    the mmap-friendly bundle alongside Main; the standalone Embed graph is later redirected
    to read it from there by :func:`finalize_embed_table_consolidation`.
...
def build_shared_merged_bundle(
    ...
    extend_prefill_rotary_initializers(
        shared,
        source_folder,
        min_shared_elements,
        model_file_names,
    )
    extend_decode_rotary_initializers(
        shared,
        source_folder,
        min_shared_elements,
        model_file_names,
    ...
    # otherwise it is registered here so the heavy [vocab, hidden] weight streams once into
    # the shared blob instead of a private Decoder_Embed.onnx.data sidecar.  Prepared before
    # the bundle is written so the table lands inside it.
    embed_prepared = None
    if dedup_embed:
        ...
        for node in model.graph.node:
            ...
            if node.op_type == "Cast":
                ...
                if len(branch_types) == 1:
                    inferred = next(iter(branch_types))
...
def transplant_optimized_encoder(
    ...
    if not target_encoder_nodes:
        return target
    if len(optimized_encoder.graph.input) != 2 or len(optimized_encoder.graph.output) != 1:
        raise RuntimeError(
            "Optimized Audio8-ASR Encoder must expose audio/input_ids inputs and bridge output."
        )
    ...
    input_remap = {
        ...
        f"encoder_{optimized_encoder.graph.input[1].name}": "input_ids",
    }
    donor_nodes = [
