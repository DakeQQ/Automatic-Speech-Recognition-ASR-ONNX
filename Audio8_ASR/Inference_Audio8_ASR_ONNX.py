"""Run Audio8-ASR with merged prefill/decode ONNX graphs and shared weights."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run merged Audio8-ASR ONNX inference.")
    parser.add_argument(
        "--onnx-folder",
        "--model-folder",
        dest="onnx_folder",
        type=Path,
        default=_SCRIPT_DIR / "Audio8_ASR_Optimized",
...
PENALTY_RANGE = 10
REPEAT_PENALTY = 1.0                # 1.0 selects greedy; another value selects penalty-greedy.
...
def build_audio8_prompt_ids(
    special_token_ids: dict[str, object],
    ...
    *,
    ...
    if input_is_normalised:
        ...
        if USE_NORMALISE_AUDIO:
            audio = audio * np.float32(audio_pcm_scale)
        ...
        if target_dtype == np.dtype(np.int16):
            audio = np.rint(audio * np.float32(audio_pcm_scale))
            np.clip(
                audio,
                -float(audio_pcm_scale),
                float(audio_pcm_scale) - 1.0,
...
def main() -> None:
    ...
    audio_hop_length = int(metadata["audio_hop_length"])
    merge_factor = int(metadata["merge_factor"])
    prompt_token_ids = [int(token_id) for token_id in json.loads(metadata["prompt_token_ids"])]
    ...
    required_special_roles = {
        "stop", "audio", "user", "audio_begin", "audio_end", "assistant"
    }
    missing_special_roles = required_special_roles - set(special_token_ids)
    if missing_special_roles:
        raise RuntimeError(
            f"Audio8 metadata is missing required token roles: {sorted(missing_special_roles)}"
        )
    ...
    for test_path in test_audio_list:
        audio_segment = AudioSegment.from_file(test_path)
        audio_waveform = np.asarray(
        ...
        if not is_dynamic_dim(audio_sample_dim):
            audio_waveform = audio_waveform[:int(audio_sample_dim)]
        if not len(audio_waveform):
            raise ValueError(f"Audio8 cannot build a prompt for empty audio: {test_path}")
        audio_slots = audio_token_count(
            len(audio_waveform), audio_hop_length, merge_factor
        )
        prompt_ids = build_audio8_prompt_ids(
            special_token_ids, prompt_token_ids, audio_slots
        )
        base_embed = _persistent_embed(
            embed_session,
            prompt_ids,
            embed_input_meta,
            embed_output_meta,
            prefill_input_meta["base_embed"],
        )
        prompt_ids_array = array_for(
            prefill_input_meta["input_ids"],
            [prompt_ids],
            axes={0: 1, 1: len(prompt_ids)},
        )
        prompt_ids_value = _ort_from_numpy(prompt_ids_array)
        audio = prepare_audio_input(
            audio_waveform.reshape(1, 1, -1),
            audio_dtype,
            audio_pcm_scale,
            input_is_normalised=False,
        )
        audio = array_for(
            audio_meta,
            audio,
        ...
        print(
            ...
            f"({len(audio_waveform) / sample_rate:.2f} s, "
            f"{audio_slots} audio slots)"
        )
        print("-" * 70)

        # One launch owns audio encoding, official audio-slot injection, rotary/mask,
        ...
        prefill_binding.bind_ortvalue_input("base_embed", base_embed)
        prefill_binding.bind_ortvalue_input("input_ids", prompt_ids_value)
        ...
        while (
            ...
            # Standalone Embed supplies both prompt and decode embeddings; only
            # the transformer stage is one merged run per generated token.
        ...
        asr_result = tokenizer.decode(generated_tokens, skip_special_tokens=True).strip()

        total_elapsed = time.time() - start_time
        rtf = total_elapsed / max(len(audio_waveform) / sample_rate, np.finfo(np.float32).tiny)
        decode_rate = decode_steps / max(decode_elapsed, np.finfo(np.float32).tiny)
        print(f"\nTranscription:\n  {asr_result}")
        print(
