"""Optimize Qwen3-ASR donors and rebuild its merged Encoder+Prefill bundle.
...
_OPTIMIZE_VARIANT = os.environ.get("MEGA_ASR_OPTIMIZE_VARIANT")
if _OPTIMIZE_VARIANT not in (None, *Shared_Merged.VARIANT_NAMES):
    raise ValueError(
        "MEGA_ASR_OPTIMIZE_VARIANT must be one of "
        f"{Shared_Merged.VARIANT_NAMES}, got {_OPTIMIZE_VARIANT!r}."
    )
_ARTIFACT_VARIANT = _OPTIMIZE_VARIANT or "mega"
_MODEL_FILE_NAMES = dict(
    Shared_Merged.VARIANT_MODEL_FILE_NAMES[_ARTIFACT_VARIANT]
)


def _model_stem(role: str) -> str:
    return Path(_MODEL_FILE_NAMES[role]).stem


_ENCODER_MODEL = _model_stem("encoder")
_MAIN_MODEL = _model_stem("main")
_EMBED_MODEL = _model_stem("embed")
_PREFILL_GREEDY_MODEL = _model_stem("prefill_greedy")
_PREFILL_PENALTY_GREEDY_MODEL = _model_stem("prefill_penalty_greedy")
_PREFILL_SAMPLING_MODEL = _model_stem("prefill_sampling")
_DECODE_GREEDY_MODEL = _model_stem("decode_greedy")
_DECODE_PENALTY_GREEDY_MODEL = _model_stem("decode_penalty_greedy")
_DECODE_SAMPLING_MODEL = _model_stem("decode_sampling")
_SHARED_INITIALIZERS_MODEL = _model_stem("shared_initializers")
_METADATA_MODEL = _model_stem("metadata")
_ROUTER_MODEL = Path(Shared_Merged.ROUTER_MODEL_NAME).stem
# ============================== USER CONFIG ==============================
...
ORIGINAL_FOLDER_PATH  = str(_SCRIPT_DIR / "Mega_ASR_ONNX")
OPTIMIZED_FOLDER_PATH = str(_SCRIPT_DIR / "Mega_ASR_Optimized")
...
# The standalone audio-quality router stays in floating point by default. Its
# graph is still optimized, but does not participate in the shared ASR bundle.
ROUTER_METHOD = "F32"
...
def _main_nodes_to_exclude(model_path: str) -> list[str] | None:
    ...
    if len(matches) != 1 or not matches[0]:
        raise RuntimeError(
            f"Expected one named Qwen3-ASR lm-head projection; found {matches}."
        )
    return matches
...
MODEL_PLANS: dict[str, Plan] = {
    ...
    _PREFILL_GREEDY_MODEL:         Plan(method="Q4", process=False, optimize=True, transformer=True),
    _PREFILL_PENALTY_GREEDY_MODEL: Plan(method="Q4", process=False, optimize=True, transformer=True),
    _PREFILL_SAMPLING_MODEL:       Plan(method="Q4", process=False, optimize=True, transformer=True),
    _DECODE_GREEDY_MODEL:          Plan(method="Q4", process=False, optimize=True, transformer=True),
    _DECODE_PENALTY_GREEDY_MODEL:  Plan(method="Q4", process=False, optimize=True, transformer=True),
    _DECODE_SAMPLING_MODEL:        Plan(method="Q4", process=False, optimize=True, transformer=True),
    _SHARED_INITIALIZERS_MODEL:    Plan(method="Q4", process=False, optimize=True, transformer=True),
    _METADATA_MODEL:               Plan(method="F32", process=False, optimize=True, transformer=True),
    _ROUTER_MODEL: Plan(
...
def _inspect_embed_layout(
    ...
    if len(gather_indices) != 1:
        raise RuntimeError(
            "Qwen3-ASR Embed must contain exactly one standard Gather; "
    ...
    if len(gather.input) < 2 or len(gather.output) != 1:
        raise RuntimeError("Qwen3-ASR Embed Gather has an unexpected input/output contract.")
    ...
    if table_index is None:
        raise RuntimeError("Qwen3-ASR Embed Gather does not read a constant table.")
    table = model.graph.initializer[table_index]
    if len(table.dims) != 2:
        raise RuntimeError(
            f"Qwen3-ASR Embed table must be rank 2, got {list(table.dims)}."
        )
    ...
    if gather_axis != 1:
        raise RuntimeError(
            f"Qwen3-ASR Embed Gather axis must be 0 or 1, got {gather_axis}."
        )
...
def _run_optimizer_pipeline() -> None:
    ...
    run_optimizer(
        ...
        reset_output_folder=_OPTIMIZE_VARIANT == "base",
    )
...
def _share_block_quantized_embed_with_main(
    ...
    if (
        ...
        raise RuntimeError("Raw Qwen3-ASR Embed and lm-head tables are not exactly tied.")
    ...
    if raw_embed_external or raw_main_external:
        if raw_embed_external != raw_main_external:
            raise RuntimeError(
                "Raw Qwen3-ASR Embed and lm-head tables reference different bytes."
            )
    elif raw_table.raw_data != raw_lm_head.raw_data:
        raise RuntimeError("Raw Qwen3-ASR Embed and lm-head tables are not byte-identical.")
...
def _share_float_embed_with_main(
    ...
    if (
        ...
        raise RuntimeError("Raw Qwen3-ASR Embed and lm-head tables are not tied.")
    raw_embed_external = _external_data_map(raw_table)
    raw_main_external = _external_data_map(raw_lm_head)
    if raw_embed_external or raw_main_external:
        if raw_embed_external != raw_main_external:
            raise RuntimeError("Raw Qwen3-ASR Embed and lm-head reference different bytes.")
    elif raw_table.raw_data != raw_lm_head.raw_data:
        raise RuntimeError("Raw Qwen3-ASR Embed and lm-head are not byte-identical.")
...
def _share_tied_embed_with_main(
    ...
    raise RuntimeError(
        f"Tied Qwen3-ASR Embed does not support Main method {main_plan.method!r}."
    )
...
def _assert_router_contract(path: Path) -> None:
    model = onnx.load(str(path), load_external_data=False)
    inputs = list(model.graph.input)
    if len(inputs) != 1 or inputs[0].name != "waveform":
        raise RuntimeError(f"Mega-ASR router must expose one waveform input, got {[value.name for value in inputs]!r}.")
    dimensions = [
        int(dimension.dim_value) if dimension.HasField("dim_value") else dimension.dim_param or None
        for dimension in inputs[0].type.tensor_type.shape.dim
    ]
    if len(dimensions) != 3 or dimensions[:2] != [1, 1]:
        raise RuntimeError(
            "Mega-ASR router waveform input must be rank-3 mono [1, 1, audio_samples], "
            f"got {dimensions!r}."
        )
    outputs = list(model.graph.output)
    if len(outputs) != 1 or outputs[0].name != Shared_Merged.ROUTER_DEGRADED_PROBABILITY_OUTPUT:
        raise RuntimeError(
            "Mega-ASR router must expose the direct degraded-probability output "
            f"{Shared_Merged.ROUTER_DEGRADED_PROBABILITY_OUTPUT!r}, "
            f"got {[value.name for value in outputs]!r}."
        )
    output_dimensions = [
        int(dimension.dim_value)
        if dimension.HasField("dim_value")
        else dimension.dim_param or None
        for dimension in outputs[0].type.tensor_type.shape.dim
    ]
    if output_dimensions != [1]:
        raise RuntimeError(
            "Mega-ASR router degraded-probability output must have shape [1], "
            f"got {output_dimensions!r}."
        )


def _verify_optimized_router(target_folder: Path) -> None:
    router_path = target_folder / Shared_Merged.ROUTER_MODEL_NAME
    if not router_path.is_file():
        raise FileNotFoundError(
            "The optimized Mega-ASR router was not produced: "
            f"{router_path}."
        )
    _assert_router_contract(router_path)
...
def build_optimized_merged_bundle() -> dict[str, Path]:
    ...
    _verify_optimized_router(output_folder)
    return generated


def _optimize_all_variants() -> None:
    output_folder = Path(OPTIMIZED_FOLDER_PATH)
    if output_folder.exists():
        shutil.rmtree(output_folder)
    for variant in Shared_Merged.VARIANT_NAMES:
        environment = dict(os.environ)
        environment["MEGA_ASR_OPTIMIZE_VARIANT"] = variant
    ...
    deduplication = Shared_Merged.deduplicate_variant_shared_initializers(
        output_folder,
        Shared_Merged.VARIANT_MODEL_FILE_NAMES["base"],
        Shared_Merged.VARIANT_MODEL_FILE_NAMES["mega"],
    )
    print(
        "[SharedMerged] Reused "
        f"{deduplication['shared_initializer_count']} unchanged optimized Mega "
        f"initializer(s) from Base ({deduplication['shared_data_bytes'] / 2**30:.2f} GiB); "
        f"saved {deduplication['saved_data_bytes'] / 2**30:.2f} GiB."
    )


if __name__ == "__main__":
    if _OPTIMIZE_VARIANT is None:
        _optimize_all_variants()
    else:
        ...
        print(
            "\n--- All standalone and merged Mega-ASR models processed "
            f"successfully ({_OPTIMIZE_VARIANT})! ---"
        )
