"""Build Qwen3-ASR merged ONNX graphs backed by one shared initializer bundle.

Each prefill graph composes the audio encoder, optional language-tail concat,
rotary/mask shell, decoder Main, and selected decode head. Decode graphs compose
rotary, Main, and the selected head. Encoder and Main weight names remain stable
so their large initializers are stored once and optimized standalone donors can be
transplanted back into every strategy graph.
"""
...
METADATA_MODEL_NAME = "Mega_ASR_Metadata.onnx"
ROUTER_MODEL_NAME = "Mega_ASR_Audio_Quality_Router.onnx"
ROUTER_DEGRADED_PROBABILITY_OUTPUT = "router_degraded_probability"
VARIANT_NAMES = ("base", "mega")


def _variant_model_file_names(variant: str) -> dict[str, str]:
    if variant not in VARIANT_NAMES:
        raise ValueError(f"Unsupported Mega-ASR variant: {variant!r}")
    prefix = f"Mega_ASR_{variant.title()}"
    shared_name = f"{prefix}_SharedInitializers.onnx"
    return {
        "metadata": METADATA_MODEL_NAME,
        "encoder": f"{prefix}_Encoder.onnx",
        "embed": f"{prefix}_Decoder_Embed.onnx",
        "concat_embed": f"{prefix}_Concat_Embed.onnx",
        "main": f"{prefix}_Decoder_Main.onnx",
        "rotary_prefill": f"{prefix}_Rotary_Mask_Text_Prefill.onnx",
        "rotary_decode": f"{prefix}_Rotary_Mask_Text_Decode.onnx",
        ...
        "greedy": f"{prefix}_Argmax.onnx",
        "penalty_greedy": f"{prefix}_Greedy_Search.onnx",
        "penalty": f"{prefix}_Apply_Penalty.onnx",
        "sampling": f"{prefix}_TopKTopPSampling.onnx",
        "prefill_greedy": f"{prefix}_Prefill_Greedy.onnx",
        "prefill_penalty_greedy": f"{prefix}_Prefill_Penalty_Greedy.onnx",
        "prefill_sampling": f"{prefix}_PrefillSampling.onnx",
        "decode_greedy": f"{prefix}_Decode_Greedy.onnx",
        "decode_penalty_greedy": f"{prefix}_Decode_Penalty_Greedy.onnx",
        "decode_sampling": f"{prefix}_DecodeSampling.onnx",
        "shared_initializers": shared_name,
        "shared_initializers_data": shared_name + ".data",
    }


VARIANT_MODEL_FILE_NAMES = {
    variant: _variant_model_file_names(variant) for variant in VARIANT_NAMES
}


def _validate_variant_artifact_isolation() -> None:
    base_names = set(VARIANT_MODEL_FILE_NAMES["base"].values())
    mega_names = set(VARIANT_MODEL_FILE_NAMES["mega"].values())
    shared_names = (base_names & mega_names) - {METADATA_MODEL_NAME}
    if shared_names:
        raise RuntimeError(
            "Base and Mega-ASR artifacts must not share filenames: "
            f"{sorted(shared_names)}"
        )


_validate_variant_artifact_isolation()

# The generic helpers retain a default for direct use, but all Mega-ASR callers
# select an explicit variant registry.
DEFAULT_MODEL_FILE_NAMES = VARIANT_MODEL_FILE_NAMES["mega"]
...
SHARED_INITIALIZER_CONSUMER_MODEL_KEYS = (
    "encoder",
    "embed",
    "main",
    "prefill_greedy",
...
def save_shared_initializers_from_tensors(shared: dict[str, TensorProto], path: Path) -> None:
    ...
    graph = onnx.helper.make_graph([], "qwen3_asr_shared_initializers", [], [], initializer=references)
    model = onnx.helper.make_model(
        graph,
        producer_name="Qwen_ASR/Shared_Merged.py",
        opset_imports=[onnx.helper.make_opsetid("", 20)],
    )
    model.ir_version = 10
    model.metadata_props.add(key="qwen_asr_shared_initializers", value="1")
...
def _external_tensor_region(
    manifest_path: Path, initializer: TensorProto
) -> tuple[Path, int, int]:
    external = _external_data_map(initializer)
    ...
    if Path(location).is_absolute():
        raise RuntimeError(
            f"Shared initializer {initializer.name!r} has an absolute data path."
        )
    try:
        ...
        raise RuntimeError(
            f"Shared initializer {initializer.name!r} has invalid external-data metadata."
        ) from error
    data_path = (manifest_path.parent / location).resolve()
    root = manifest_path.parent.resolve()
    if not data_path.is_relative_to(root):
        raise RuntimeError(
            f"Shared initializer {initializer.name!r} escapes its artifact folder."
        )
    if offset < 0 or length < 0 or offset + length > data_path.stat().st_size:
        raise RuntimeError(
            f"Shared initializer {initializer.name!r} has an invalid data range."
        )
    return data_path, offset, length


def _external_region_digest(
    data_path: Path,
    offset: int,
    length: int,
    cache: dict[tuple[Path, int, int], bytes],
) -> bytes:
    key = (data_path, offset, length)
    ...
    with data_path.open("rb") as data_file:
        ...
        while remaining:
            ...
            if not chunk:
                raise RuntimeError(
                    f"Unexpected end of {data_path} while hashing shared data."
                )
            hasher.update(chunk)
            remaining -= len(chunk)
...
def _copy_external_region(
    ...
    with source_path.open("rb") as source:
        ...
        while remaining:
            ...
            if not chunk:
                raise RuntimeError(
                    f"Unexpected end of {source_path} while compacting shared data."
                )
            destination.write(chunk)
            remaining -= len(chunk)


def deduplicate_variant_shared_initializers(
    folder: Path,
    reference_model_file_names: dict[str, str],
    target_model_file_names: dict[str, str],
) -> dict[str, int]:
    """Reuse byte-identical target tensors from a reference variant's data blob.

    Base and Mega-ASR need separate graphs because the router selects between
    their different weights. This pass keeps that ABI intact while compacting the
    target shared-data blob: only tensors whose full SHA-256 digest matches a
    reference region are redirected to the reference blob. All target consumers
    are rewritten to the updated external-data offsets.
    """
    folder = Path(folder)
    reference_manifest_path = folder / _model_file_name(
        reference_model_file_names, "shared_initializers"
    )
    target_manifest_path = folder / _model_file_name(
        target_model_file_names, "shared_initializers"
    )
    if reference_manifest_path == target_manifest_path:
        raise ValueError("Reference and target shared-initializer manifests must differ.")
    ...
    reference_manifest = load_model(reference_manifest_path, load_external_data=False)
    target_manifest = load_model(target_manifest_path, load_external_data=False)
    digest_cache: dict[tuple[Path, int, int], bytes] = {}
    reference_regions: dict[tuple[int, bytes], dict[str, str]] = {}
    target_root = target_manifest_path.parent.resolve()

    for initializer in reference_manifest.graph.initializer:
        data_path, offset, length = _external_tensor_region(
            reference_manifest_path, initializer
        )
        try:
            location = str(data_path.relative_to(target_root))
        except ValueError as error:
            raise RuntimeError(
                "Cross-variant shared data must stay inside one artifact folder."
            ) from error
        digest = _external_region_digest(data_path, offset, length, digest_cache)
        reference_regions.setdefault(
            (length, digest),
            {
                "location": location,
                "offset": str(offset),
    ...
    target_data_path = target_manifest_path.with_name(
        target_manifest_path.name + ".data"
    )
    original_target_bytes = (
        target_data_path.stat().st_size if target_data_path.exists() else 0
    )
    temporary_data_path = target_data_path.with_name(
        target_data_path.name + ".dedup.tmp"
    )
    temporary_data_path.unlink(missing_ok=True)

    target_regions: dict[tuple[int, bytes], dict[str, str]] = {}
    new_initializers: list[TensorProto] = []
    shared_region_keys: set[tuple[int, bytes]] = set()
    shared_initializer_count = 0
    changed_initializer_count = 0

    with temporary_data_path.open("wb") as compacted_data:
        for initializer in target_manifest.graph.initializer:
            data_path, offset, length = _external_tensor_region(
                target_manifest_path, initializer
            )
            digest = _external_region_digest(data_path, offset, length, digest_cache)
            region_key = (length, digest)
            external = reference_regions.get(region_key)
            if external is not None:
                shared_region_keys.add(region_key)
                shared_initializer_count += 1
            else:
                external = target_regions.get(region_key)
                if external is None:
                    external = {
                        "location": target_data_path.name,
                        "offset": str(compacted_data.tell()),
                        "length": str(length),
                    }
                    _copy_external_region(data_path, offset, length, compacted_data)
                    target_regions[region_key] = external
            old_external = _external_data_map(initializer)
            if any(old_external.get(key) != value for key, value in external.items()):
                changed_initializer_count += 1
            new_initializers.append(make_external_initializer_ref(initializer, external))

    compacted_target_bytes = temporary_data_path.stat().st_size
    if not changed_initializer_count:
        temporary_data_path.unlink(missing_ok=True)
        return {
            "shared_initializer_count": shared_initializer_count,
            "shared_data_bytes": sum(key[0] for key in shared_region_keys),
            "saved_data_bytes": max(0, original_target_bytes - compacted_target_bytes),
            "rewritten_graph_count": 0,
        }

    if compacted_target_bytes:
        temporary_data_path.replace(target_data_path)
    else:
        temporary_data_path.unlink(missing_ok=True)

    del target_manifest.graph.initializer[:]
    target_manifest.graph.initializer.extend(new_initializers)
    onnx.save_model(
        target_manifest,
        str(target_manifest_path),
        save_as_external_data=False,
    )
    external_by_name = shared_external_data_map(target_manifest_path)

    rewritten_graph_count = 0
    for role in SHARED_INITIALIZER_CONSUMER_MODEL_KEYS:
        graph_path = folder / _model_file_name(target_model_file_names, role)
        if not graph_path.exists():
            continue
        graph = load_model(graph_path, load_external_data=False)
        if redirect_shared_initializers_to_external(graph, external_by_name):
            onnx.save_model(graph, str(graph_path), save_as_external_data=False)
            rewritten_graph_count += 1

    if not compacted_target_bytes:
    ...
    return {
        ...
        "shared_data_bytes": sum(key[0] for key in shared_region_keys),
        "saved_data_bytes": max(0, original_target_bytes - compacted_target_bytes),
        "rewritten_graph_count": rewritten_graph_count,
    }
...
def build_prefill_frontend(
    ...
    """Compose Encoder + optional language-tail concat for a prefill graph."""
    if encoder is None:
        encoder = load_model(
    ...
    concat = load_model(
        source_folder / _model_file_name(model_file_names, "concat_embed")
    )
    encoder_component = prefixed_preserving_initializers(encoder, "encoder_")
    concat_component = prefixed(concat, "concat_")
    encoder_hidden = f"encoder_{encoder.graph.output[0].name}"
    concat_base = f"concat_{concat.graph.input[0].name}"
    concat_tail = f"concat_{concat.graph.input[1].name}"
    concat_hidden = f"concat_{concat.graph.output[0].name}"
    concat_len = f"concat_{concat.graph.output[1].name}"
    frontend = merge_models_no_check(
        encoder_component,
        concat_component,
        [(encoder_hidden, concat_base)],
    )
    rename_tensor(frontend, f"encoder_{encoder.graph.input[0].name}", "audio")
    rename_tensor(
        frontend, f"encoder_{encoder.graph.input[1].name}", "query_embed"
    )
    rename_tensor(frontend, concat_tail, "language_tail_embed")
    return frontend, concat_hidden, concat_len
...
def merge_models_no_check(
    ...
    merged.producer_name = "Qwen_ASR/Shared_Merged.py"
...
def make_merged_build_plan(
    ...
    return [
        (
            ...
            [name("encoder"), name("concat_embed"), name("rotary_prefill"), name("greedy")],
        ),
        (
            ...
            [name("encoder"), name("concat_embed"), name("rotary_prefill"), name("penalty_greedy")],
        ),
        (
            ...
            [name("encoder"), name("concat_embed"), name("rotary_prefill"), name("sampling")],
        ),
        (
            name("decode_greedy"),
...
def references_shared_bundle(
    model_path: Path, shared_model_path: Path
) -> bool:
    """True if a graph initializer exactly references this shared manifest.

    A routed variant may borrow unchanged tensors from another variant's data
    blob, so matching only one ``.data`` filename is insufficient. Compare the
    external location and range against the manifest entry instead.
    """
    external_by_name = shared_external_data_map(Path(shared_model_path))
    model = load_model(Path(model_path), load_external_data=False)
    for initializer in model.graph.initializer:
        expected = external_by_name.get(initializer.name)
        if expected is None or initializer.data_location != TensorProto.EXTERNAL:
            continue
        expected_dims = tuple(
            int(dim) for dim in expected["__tensor_dims"].split(",") if dim
        )
        if (
            initializer.data_type != int(expected["__tensor_data_type"])
            or tuple(initializer.dims) != expected_dims
        ):
            continue
        actual = _external_data_map(initializer)
        if all(
            actual.get(key) == expected.get(key)
            for key in ("location", "offset", "length")
        ):
            return True
    return False
...
def delete_obsolete_strategy_artifacts(
    ...
    old_family = "Be" + "am"
    legacy_names = (
        f"Qwen3_ASR_First_{old_family}_Search.onnx",
        f"Qwen3_ASR_Second_{old_family}_Search.onnx",
        f"Qwen3_ASR_Prefill_{old_family}.onnx",
        f"Qwen3_ASR_Decode_{old_family}.onnx",
        f"Qwen3_ASR_Decode_Penalty_{old_family}.onnx",
    )
    protected = {
...
def dedup_embed_into_shared_lm_head(
    ...
    (``[hidden, vocab]`` vs ``[vocab, hidden]``) and, because Qwen3-ASR ties
...
def prepare_embed_table_for_sharing(
    ...
    tied to Main's ``lm_head``.  Qwen3-ASR unties them, so the heavy ``[vocab, hidden]``
...
def build_shared_merged_bundle(
    ...
    # otherwise (Qwen3-ASR unties them) it is registered here so the heavy [vocab, hidden]
...
def _target_rotary_remap(
    ...
    if not target_is_decode and "hidden_states" in donor_inputs:
        ...
        concat_outputs = {
            ...
            for node in target.graph.node
            for output in node.output
            if output.startswith("concat_")
        }
        hidden_candidates = sorted(concat_outputs & target_main_inputs)
...
def transplant_optimized_encoder(
    ...
    if len(optimized_encoder.graph.input) != 2 or not optimized_encoder.graph.output:
        raise RuntimeError(
            "Optimized Qwen3-ASR Encoder must expose audio/query inputs and concat output."
        )
    ...
    input_remap = {
        ...
        f"encoder_{optimized_encoder.graph.input[1].name}": "query_embed",
    }
    donor_nodes = [
